      ...
      <div className="w-full max-w-[1300px] text-center">
        ...
        <div className="grid grid-cols-4 gap-[30px] max-[1100px]:grid-cols-2 max-[1100px]:gap-[25px] max-[640px]:grid-cols-1 max-[640px]:gap-[30px]">
          {releases.map((release) => (
            <div key={release.id} className="flex flex-col items-center rounded-[20px] bg-white/3 px-5 pb-[30px] pt-[25px] text-center transition-all duration-300 hover:bg-white/6 hover:-translate-y-[5px]">
              ...
              <p className="mb-3 font-sans text-[14px] font-normal text-[#aaa]">{release.artist}</p>
              <p className="mb-5 flex-grow font-sans text-[14px] font-light leading-[1.6] text-[#ddd]">{release.description}</p>
              <a 
                href={release.bandcampUrl} 
                target="_blank" 
                rel="noopener noreferrer"
                className="inline-block w-full rounded-xl bg-[#6d28d9] px-0 py-[14px] text-center font-sans text-[14px] font-bold uppercase tracking-[0.5px] text-white no-underline transition-all duration-300 hover:scale-103 hover:bg-[#7c3aed] hover:shadow-[0_0_25px_rgba(109,40,217,0.6)]"
              >
