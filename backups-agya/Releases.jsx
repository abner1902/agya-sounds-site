...
export default function Releases() {
  ...
  return (
    <section 
      id="lancamentos" 
      className="flex min-h-screen items-center justify-center bg-black px-[60px] py-[100px] max-[1100px]:px-5 max-[1100px]:py-[60px]"
    >
      <div className="w-full max-w-[1300px] text-center">
        
        <h2 className="mb-6 font-sans text-[40px] font-black uppercase text-white max-md:text-[32px]">
          ÚLTIMOS LANÇAMENTOS
        </h2>
        ...
        <div className="grid grid-cols-4 gap-[30px] max-[1100px]:grid-cols-2 max-[1100px]:gap-[25px] max-[640px]:grid-cols-1 max-[640px]:gap-[30px]">
          {releases.map((release) => (
            <div 
              key={release.id}
              className="flex flex-col items-center rounded-[20px] bg-white/[0.03] px-5 pb-[30px] pt-[25px] text-center transition-all duration-300 hover:bg-white/[0.06] hover:-translate-y-[5px]"
            >
              ...
              <h3 className="mb-2 font-sans text-[20px] font-bold text-white max-md:text-[18px]">
                {release.title}
              </h3>

              <p className="mb-3 font-sans text-[14px] font-normal text-[#aaa]">
                {release.artist}
              </p>

              <p className="mb-5 flex-grow font-sans text-[14px] font-light leading-[1.6] text-[#ddd]">
                {release.description}
              </p>
